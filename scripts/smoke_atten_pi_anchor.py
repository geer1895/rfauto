"""临时冒烟：π 型电阻衰减器真跑（10dB/50Ω，串 71.15Ω + 两端对地 96.25Ω）。

锚判据（WP2.3）：
1. β 金标准：CalcPort β → εeff 对照 skrf HJ ±2%（mesh=0.4mm 收敛档）；
2. 衰减平坦：|S21| @2.5GHz 对照目标 -10dB ±0.5dB（ABCD 电阻网络
   确定性裁判同源）；
3. 匹配：|S11|max <-12dB（商用 lumped 衰减模块回损规格地板
   12~18dB 口径；理想 pad 匹配，实测残余= lumped 寄生）。
工作目录参数化（#198 教训）。证据链 runs/atten_pi_smoke/。
"""
import argparse
import csv
import sys
...
work = Path(f"runs/atten_pi_smoke/{args.pt}")
...
assert solver.build_geometry({"template": "atten_pi",
...
print(f"ATTEN_PI_PROBE_{verdict}（判据：|Δεeff|≤2% HJ、"
