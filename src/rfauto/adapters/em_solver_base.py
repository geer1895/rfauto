...
class EMSolverType(str, Enum):
  ...
  NGSOLVE = "ngsolve" # 开源 FEM（A9 零 license 频域通道；）
  ICEPAK = "icepak" # AEDT 热通道（WP4.4a 电-热首案例；license 探测 ✅）
  Q3D = "q3d" # AEDT 寄生提取通道（WP4.4b PCB 无源/互连 RLC；license 探测 ✅）
  FAKE = "fake"  # 解析近似（测试用）
...
class EMSolverConfig:
  ...
  # openEMS 新语义（官方方法学）：网格 base 覆盖（mm）；
