...
- 非有限值（NaN/±Inf，params/metrics/cost 三处同口径）在收集侧整点拦截
 ...
 永不落盘（数据面遗留②根治；DuckDB read_parquet 统计路径下 ``cost < 阈值``
...
- where 是单条 SQL WHERE 片段，service 层做白名单字符集 + 注入子串
 ...
 参数绑定下推，值永不拼进 SQL（数据面遗留③：limit 在过滤后生效）；
...
# 真机引擎白名单词根：HFSS 为对齐基准（基准铁律），openEMS/COMSOL 为
...
def _canonical_json(obj: dict[str, Any]) -> str:
  ...
  allow_nan=False（数据面遗留②根治）：非有限值（NaN/±Inf）在收集侧已被
...
def _collect_run_points(
  ...
  数据面遗留②根治：params/metrics/cost 含非有限值（NaN/±Inf，含
  ...
  def _add(params: Any, metrics: Any, cost: Any, point_index: int,
    ...
    if _has_nonfinite(params) or _has_nonfinite(
      ...
      # NaN JSON 字面量根治（数据面遗留②）：整点拦截 + 计数，不落盘
      n_nonfinite += 1
      errors.append(
  ...
  # ⑤ 单次 run 产物（api.run_once 通道，WP3.4 patch HFSS GT 战役）：
  # ①—④ 均无产物且 meta.status=done 且 results/metrics.json 存在时，
  # 整个 run 物化为一个点——params 取 recipe.snapshot.yaml 设计变量
  # （_single_run_params，与注册表 GT 行参数列同口径），metrics 取
  # results/metrics.json 的 metrics 段，cost 无统一语义置 NULL（可空
  # 列既有契约），provenance 带 run_id；results/params.sNp 在时再带
  # touchstone_path（相对 run 目录）+ n_ports（int）——数据工厂曲线级
  # 关联（曲线级神经算子 A/B 铺路），无 Touchstone 的 run 不带这两键
  # （provenance 逐字节不变）。既有 ①—④ 产物在时不触发（不重复计点）；
  # 其余 run 类型行为不变。
...
def materialize_dataset(
  ...
  *,
  ...
  n_nonfinite_skipped = 0 # 数据面遗留②：非有限值整点拦截计数（含 Infinity）
  raw_points: list[dict[str, Any]] = [] # 内部行：含原始 params dict
  run_infos: list[dict[str, Any]] = []

  for rid in target_ids:
    # G11 门禁（best-effort，#105）：unhealthy（真实 FAIL 证据）禁入
    # 注册表。注意只拦 verdict=="unhealthy"——"无可识别体检产物"的
    # run（如纯校准 samples）是 suspect-by-absence，不是健康问题，
    # 拦掉会误杀整个校准数据集（#209）；verdict 仍记录进 manifest
    # 供查询侧降权/过滤。
    if health_gate:
      try:
        from rfauto.service.health_service import health_check_run

        hc = health_check_run(rid)
        verdict = str(hc.get("verdict") or "unknown")
        health_verdicts[rid] = verdict
        if verdict == "unhealthy":
          unhealthy_runs.append(rid)
          continue
      except Exception as exc:
        # 体检器自身故障不阻塞物化，如实降级为不拦
        collect_errors.append(f"health_gate {rid}: {exc}")
    points, errs, n_nf = _collect_run_points(runs_root / rid)
    collect_errors.extend(errs)
    n_nonfinite_skipped += n_nf
    if not points:
      skipped_runs.append(rid)
      continue
    try:
      meta = json.loads(
        (runs_root / rid / "meta.json").read_text(encoding="utf-8"))
    except Exception:
      meta = {}
    provenance = {
    ...
    run_infos.append({"run_id": rid, "n_points": len(points)})
    for pt in points:
      try:
      ...
      raw_points.append({
        ...
        "study_name": str(meta.get("study_name") or ""),
...
def query_dataset(
  ...
  *,
  ...
  且在 LIMIT 之前生效（数据面遗留③：collect 按族过滤时不再被过滤前
