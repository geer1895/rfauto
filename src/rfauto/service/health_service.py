...
- 功率守恒（§10.21 D3-1/G11）：dump_type=29 体 dump（infra/loss_dump 逐
...
# DP-11：测量 run（adapter="vna"）校准态因子（服务层分支；内核不动）。
# WARN 上浮由 health_check_run 的测量分支消费（未校准 → suspect）。
FACTOR_MEASUREMENT_CALIBRATION = "measurement_calibration"

# P2⑲ 探针频带：TEMPLATE_META f0 × [0.8, 1.2]，201 点（覆盖设计带与近邻，
...
def _parse_sparams_csv_masked(
    ...
    独立测得，置零/补齐元素与已测元素比对是假阳性（2026-09-17 数据入库批
...
# ---------------------------------------------------------------------------
# G11 补强（§10.21 D3-1）：损耗 dump 功率 + 热合理性产物发现
# ---------------------------------------------------------------------------
...
def _vna_measurement_factor(run_dir: Path) -> dict[str, Any]:
    """DP-11：vna run 校准态因子（读 vna_measure.json，best-effort #105）。

    - vna_measure.json 缺失/损坏 → WARN（测量 meta 缺失，#316 多报方向）；
    - calibration.calibrated False/缺失 → WARN（G4 决议：未校准 → suspect）；
    - 校准 on → PASS（门禁口径不变）。
    """
    raw = _read_json(Path(run_dir) / "vna_measure.json")
    if raw is None:
        return {
            "factor": FACTOR_MEASUREMENT_CALIBRATION,
            "status": WARN,
            "detail": "vna run 缺 vna_measure.json（测量 meta 缺失，"
                      "校准态不可证实）",
            "lesson_ref": "DP-11 G4（规格书）：测量 meta 强制校准状态",
        }
    cal = raw.get("calibration") if isinstance(raw, dict) else None
    calibrated = bool(cal.get("calibrated")) if isinstance(cal, dict) else False
    suspects = raw.get("suspect") if isinstance(raw, dict) else None
    detail = (f"校准={'on' if calibrated else 'OFF'}"
              f"（calkit={(cal or {}).get('calkit_id', '') or 'n/a'}）")
    if isinstance(suspects, list) and suspects:
        detail += f"；suspect 记录 {len(suspects)} 条"
    return {
        "factor": FACTOR_MEASUREMENT_CALIBRATION,
        "status": PASS if calibrated else WARN,
        "detail": detail if calibrated else
                  f"测量未校准（DP-11 G4：未校准 → suspect）；{detail}",
        "lesson_ref": "DP-11 G4（规格书）：测量 meta 强制校准状态",
    }
...
def health_check_run(run_id: str, *, runs_dir: str | Path | None = None) -> dict[str, Any]:
    ...
    # DP-11：测量 run 分支（meta adapter="vna"）——校准态因子追加。
    # 未校准/测量 meta 缺失 → WARN → verdict=suspect（规格书 G4 决议）；
    # 门禁口径不变：仅 verdict=="unhealthy"（真实 FAIL 证据）禁入注册表。
    if meta and str(meta.get("adapter") or "") == "vna":
        vna_factor = _vna_measurement_factor(run_dir)
        report["factors"].append(vna_factor)
        if vna_factor["status"] == WARN and report["verdict"] == VERDICT_HEALTHY:
            report["verdict"] = VERDICT_SUSPECT
            report["ok"] = False
...
# ---------------------------------------------------------------------------
# P2⑲：开跑前 fake 成本非退化探针（#195 因子前置到战役开工之前）
# ---------------------------------------------------------------------------
...
def fake_cost_degeneracy_probe(
    ...
    *,
    ...
    """开跑前 fake 成本非退化探针（P2⑲）：#195 cost 退化因子前置。
